/**
 * Kitty Unicode placeholder helpers.
 *
 * Adapted from pi-tmux-images (MIT), which adapted the protocol structure
 * from pi-sprite (MIT). See NOTICE for attribution.
 */
...
export function upload(base64: string, imageId: number, format: number, inTmux: boolean): string[] {
	...
	return chunks.map((chunk, index) =>
		kitty(`${index ? "" : `a=t,f=${format},i=${imageId},q=2,`}m=${index + 1 < chunks.length ? 1 : 0};${chunk}`, inTmux),
	);
}
...
export function placement(
	...
	const cropControls = [
		...
		crop.width !== undefined ? `w=${crop.width}` : "",
		crop.height !== undefined ? `h=${crop.height}` : "",
	].filter(Boolean);
	const suffix = cropControls.length > 0 ? `,${cropControls.join(",")}` : "";
	return kitty(
		`a=p,i=${imageId},p=${placementId(imageId)},U=1,c=${columns},r=${rows},q=2${suffix};`,
		inTmux,
	);
}
